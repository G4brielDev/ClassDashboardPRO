...
beforeEach(async () => { directory = await mkdtemp(path.join(tmpdir(), 'tela-de-sala-')); });
afterEach(async () => { await rm(directory, { recursive: true, force: true }); });

describe('API Express', () => {
  it('responde ao health check', async () => {
    const app = createApp(new JsonStateStore(path.join(directory, 'state.json')));
  ...
  it('inicializa e persiste o estado', async () => {
    ...
    const changed = { ...initial.body, revision: initial.body.revision + 1, lastUpdated: new Date().toISOString(), school: { ...initial.body.school, classroomName: '8º Ano B' } };
